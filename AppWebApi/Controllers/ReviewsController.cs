...
namespace AppWebApi.Controllers
{
    ...
    {
        readonly IReviewsService _service = null;
        ...
        public async Task<IActionResult> ReadItem(string id = null, string flat = "false")
        {
            try
            {
                ...
                if (!checkGuid)
                {
                    return BadRequest(new ResponseItemDto<ReviewsCuDto>
                    {
        ...
        //DELETE: api/reviews/deleteitem/id
        [HttpDelete("{id}")]
        ...
        //GET: api/reviews/readitemdto
        [HttpGet()]
        [ActionName("ReadItemDto")]
        [ProducesResponseType(200, Type = typeof(ReviewsCuDto))]
        [ProducesResponseType(400, Type = typeof(string))]
        [ProducesResponseType(404, Type = typeof(string))]
        public async Task<IActionResult> ReadItemDto(string id = null)
        {
            try
            {
                ...
                var item = await _service.ReadReviewAsync(idArg, false);
                if (item == null) throw new ArgumentException($"Item with id {id} does not exist");

                return Ok(
                    new ResponseItemDto<ReviewsCuDto>()
                    {
#if DEBUG
                        ConnectionString = item.ConnectionString,
#endif
                        Item = new ReviewsCuDto(item.Item)
                    });
        ...
        [ProducesResponseType(200, Type = typeof(IReviews))]
        [ProducesResponseType(400, Type = typeof(string))]
        public async Task<IActionResult> UpdateItem(string id, [FromBody] ReviewsCuDto item)
        {
            try
            {
                var checkGuid = Guid.TryParse(id, out var guid);
                ...
                if (item.ReviewId != idArg)
                {
                    return BadRequest(new ResponseItemDto<ReviewsCuDto>
                    {
                        ErrorMessage = $"Input id {idArg} does not match item id {item.ReviewId}"
                    }
                    );
                }
                var model = await _service.UpdateReviewAsync(item);
        ...
        public ReviewsController(IReviewsService service, ILogger<ReviewsController> logger)
        {
            _service = service;
            _logger = logger;
        }
